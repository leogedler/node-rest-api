...
workers.validateCheckData = originalCheckData => {
  ...
  // Set the keys that may not be set (if the workers have never seen this check begore)
  ...
  // If all the checks pass, pass the data along to the next step on the process
  if (
    originalCheckData.id &&
    originalCheckData.userPhone &&
    originalCheckData.protocol &&
...
workers.alertUserToStatusChange = (newCheckData)=>{
  ...
  helpers.sendTwilioSms(newCheckData.userPhone, msg, (err)=>{
    if(!err){
      console.log('Success: User was alerted to an status change in their checks via sms');
...
workers.init = () => {
  ...
  // Call the loop so the checks will execute
  workers.loop();
};
